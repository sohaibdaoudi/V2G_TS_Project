...
def create_excel_report(results_with_v2g, results_without_v2g, date_range, load_pred, 
                        forecast_days, diesel_prices, v2g_price):
    """
    ...
    diesel_prices : array-like
        Hourly diesel prices (constant or time-varying)
    v2g_price : float
    ...
    # Diesel price information
    if np.allclose(diesel_prices, diesel_prices[0]):
        diesel_price_desc = f"Constant: {diesel_prices[0]:.2f} MAD/MWh"
    else:
        diesel_price_desc = f"Time-varying: {min(diesel_prices):.2f} - {max(diesel_prices):.2f} MAD/MWh"
    ...
    if results_with_v2g and results_without_v2g:
        ...
        percent_savings = (cost_savings / results_without_v2g['total_cost']) * 100 / 100  # Convert to decimal for Excel
    ...
    if results_with_v2g:
        df_with_v2g = pd.DataFrame({
            ...
            "Diesel Cost (MAD)": results_with_v2g['diesel_used'] * diesel_prices,
            "V2G Cost (MAD)": results_with_v2g['v2g_used'] * v2g_price,
            "Total Cost (MAD)": (results_with_v2g['diesel_used'] * diesel_prices) + 
        ...
        with_v2g_sheet.set_column('H:K', 18, currency_format)
        ...
        with_v2g_sheet.write_formula(total_row, 10, f'=SUM(K2:K{total_row})', total_currency_format)
    ...
    if results_without_v2g:
        df_without_v2g = pd.DataFrame({
            ...
            "Diesel Price (MAD/MWh)": diesel_prices,
            "Diesel Cost (MAD)": results_without_v2g['diesel_used'] * diesel_prices,
            "Total Cost (MAD)": results_without_v2g['diesel_used'] * diesel_prices
        })
        ...
        without_v2g_sheet.set_column('G:I', 18, currency_format)
        ...
        without_v2g_sheet.write_formula(total_row, 8, f'=SUM(I2:I{total_row})', total_currency_format)
    ...
    if results_with_v2g and results_without_v2g:
        df_comparison = pd.DataFrame({
            ...
            "Diesel Price (MAD/MWh)": diesel_prices,
            "Cost (with V2G) (MAD)": (results_with_v2g['diesel_used'] * diesel_prices) + 
                                    (results_with_v2g['v2g_used'] * v2g_price),
            "Cost (without V2G) (MAD)": results_without_v2g['diesel_used'] * diesel_prices,
            "Cost Savings (MAD)": (results_without_v2g['diesel_used'] * diesel_prices) - 
                                 ((results_with_v2g['diesel_used'] * diesel_prices) + 
                                  (results_with_v2g['v2g_used'] * v2g_price)),
            "Savings (%)": (((results_without_v2g['diesel_used'] * diesel_prices) - 
                           ((results_with_v2g['diesel_used'] * diesel_prices) + 
                            (results_with_v2g['v2g_used'] * v2g_price))) / 
                          (results_without_v2g['diesel_used'] * diesel_prices) * 100) / 100  # Convert to decimal for Excel
        })
        ...
        comparison_sheet.set_column('H:K', 18, currency_format)
        comparison_sheet.set_column('L:L', 12, percent_format)
        ...
        comparison_sheet.conditional_format(f'K2:K{len(df_comparison)+1}', {
            'type': '3_color_scale',
            'min_color': "#FFFFFF",
            'mid_color': "#B7E1CD",
            'max_color': "#009E73"
        ...
        comparison_sheet.write_formula(total_row, 10, f'=SUM(K2:K{total_row})', total_currency_format)
        comparison_sheet.write_formula(total_row, 11, f'=K{total_row+1}/J{total_row+1}', percent_format)
    ...
    if results_with_v2g and results_without_v2g:
        ...
        for day in unique_days:
            ...
            # Calculate daily costs with time-varying diesel prices
            day_diesel_prices = diesel_prices[indices]
            
            cost_with_v2g = sum(results_with_v2g['diesel_used'][indices] * day_diesel_prices) + sum(results_with_v2g['v2g_used'][indices]) * v2g_price
            cost_without_v2g = sum(results_without_v2g['diesel_used'][indices] * day_diesel_prices)
...
def get_user_recommendations(results_with_v2g, results_without_v2g, v2g_price, diesel_price, max_v2g_hours):
    ...
    v2g_price : float
        Price of V2G energy in MAD/MWh
    diesel_price : float
        Average price of diesel energy in MAD/MWh
    max_v2g_hours : int
