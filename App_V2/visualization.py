...
def plot_predictions_with_historical(date_range, load_pred, solar_energy_pred, ev_pred_inv, historical_data):
    ...
    fig.add_trace(
        go.Scatter(x=date_range, y=load_pred.flatten(),
                  name='Load Forecast', line=dict(color='#1E88E5', width=2)),
        row=1, col=1
    )
    fig.add_trace(
        go.Scatter(x=historical_data['date_range'], y=historical_data['load'],
                  name='Historical Load', line=dict(color='#1E88E5', width=2, dash='dot')),
        row=1, col=1
    )
    ...
    fig.add_trace(
        go.Scatter(x=date_range, y=solar_energy_pred.flatten(),
                  name='Solar Forecast', line=dict(color='#FFC107', width=2)),
        row=2, col=1
    )
    fig.add_trace(
        go.Scatter(x=historical_data['date_range'], y=historical_data['solar'],
                  name='Historical Solar', line=dict(color='#FFC107', width=2, dash='dot')),
        row=2, col=1
    )
    ...
    fig.add_trace(
        go.Scatter(x=date_range, y=ev_pred_inv.flatten(),
                  name='EV Forecast', line=dict(color='#4CAF50', width=2)),
        row=3, col=1
    )
    fig.add_trace(
        go.Scatter(x=historical_data['date_range'], y=historical_data['ev'],
                  name='Historical EV', line=dict(color='#4CAF50', width=2, dash='dot')),
        row=3, col=1
    )
...
def create_energy_distribution_charts(results_with_v2g, results_without_v2g, load_pred):
    ...
    fig.add_trace(
        go.Pie(
            ...
            marker=dict(colors=['#FFC107', '#4CAF50', '#F44336']),
            hole=0.4,
    ...
    fig.add_trace(
        go.Pie(
            ...
            textinfo="percent+label",
            marker=dict(colors=['#FFC107', '#9C27B0']),
            hole=0.4,
            hoverinfo="label+percent+value",
            hovertemplate="%{label}: %{value:.2f} MWh (%{percent})<extra></extra>",
            pull=[0, 0.05]
...
def create_cost_comparison_chart(results_with_v2g, results_without_v2g):
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                x=1.5,
                y=diesel_cost_without_v2g / 2,
                text=f"Save {percent_savings:.1f}%",
                showarrow=True,
                arrowhead=1,
                ax=0,
                ay=-40,
...
def plot_diesel_prices(diesel_df):
    """Create a plot of diesel prices over time."""
    fig = go.Figure()
    
    # Add diesel price line
    fig.add_trace(
        go.Scatter(
            x=diesel_df['Date'],
            y=diesel_df['Price'],
            ...
            line=dict(color='#F44336', width=2),
            marker=dict(size=6),
            hovertemplate="Date: %{x}<br>Price: %{y:.2f} MAD/MWh<extra></extra>"
        )
    )
